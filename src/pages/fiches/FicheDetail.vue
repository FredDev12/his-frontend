...
function mapPatientToFiche(patient) {
  return {
    ...patient,
    ...
    identification: {
      ...
      contact_urgence: {
        nom: patient.personne_a_contacter,
        telephone: patient.telephone_urgence,
        lien: null
      }
    },

    paiement: {
      statut: patient.paiement_effectue,
      montant: patient.montant_fiche,
      facture_numero: null,
      recu_numero: null,
      date_paiement: null
    }
...
const load = async () => {
  ...
  loading.value = true
  try {
    await patient.fetchPatient(id)
    fiche.value = mapPatientToFiche(patient.patient)
