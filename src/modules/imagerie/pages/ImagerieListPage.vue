...
async function loadExamens(params = {}) {
  try {
    ...
    console.error('[Imagerie] Erreur chargement:', error)
    toast.error(error.response?.data?.message || 'Impossible de charger les examens d’imagerie.')
  }
}
...
async function search(filters) {
  try {
    ...
    console.error('[Imagerie] Erreur recherche:', error)
    toast.error(error.response?.data?.message || 'Recherche imagerie impossible.')
  }
}
...
async function confirmRemove() {
  ...
  try {
    ...
    console.error('[Imagerie] Suppression impossible:', error)
  }
}
</script>
...
<template>
  <div class="space-y-6">
    <header class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
      <div>
        <h1 class="his-page-title">Imagerie</h1>

        <p class="his-page-subtitle">Demandes d’imagerie, conclusions et comptes rendus.</p>
      </div>

      <RouterLink to="/imagerie/create">
    ...
    <BaseCard>
      ...
      <ImagerieTable :examens="store.examens" :loading="store.loading" @remove="askRemove" />
    ...
    <ConfirmDialog
      :open="removeOpen"
      title="Supprimer cet examen d’imagerie"
      :message="`Cette action va supprimer la demande d’imagerie ${examenToRemove?.examen_principal || ''}. Cette action doit être auditée côté serveur.`"
